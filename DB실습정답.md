...
crew_id와 nickname 반복

2. attendance 테이블에서 중복을 제거하기 위해 crew 테이블을 만들려고 한다. 어떻게 구성해 볼 수 있을까?

crew_id(PK)와 nickname을 가지는 crew 테이블로 만들 수 있을 것 같다.
...
attendance에서 nickname 컬럼이 필요없다.
...
```sql
ALTER TABLE crew
ADD CONSTRAINT uc_crew_nickname UNIQUE (nickname);
```
...
WHERE crew_id = (SELECT crew_id FROM crew WHERE nickname = '워넬');
```
...
```sql
SELECT crew_id, COUNT(*) AS total_days
FROM attendance
GROUP BY crew_id;
```
